...
static _Atomic unsigned int gp_count = 0;
...
typedef struct {
    ...
    int gid;                 /* gp unique identifier */
} groups;

client_t *clients[MAX_CLIENTS];
groups *gps[MAX_GROUPS];
...
void send_message_to_gp_members(char *s, int gid){
    pthread_mutex_lock(&clients_mutex);
    for (int i = 0; i < MAX_CLIENTS; ++i){
        if (clients[i]) { 
            if (clients[i]->gp_ID == gid) {
                if (write(clients[i]->connfd, s, strlen(s))<0) {
                    perror("Write to descriptor failed");
                    break;
                }
...
void send_active_groups(int connfd){
    char s[64];

    pthread_mutex_lock(&groups_mutex);
    for (int i = 0; i < MAX_GROUPS ; ++i){
        if (gps[i]) {
            sprintf(s, "<< [%d] %s\r\n", gps[i]->gid, gps[i]->name);
            send_message_self(s, connfd);
        }
    }
    pthread_mutex_unlock(&groups_mutex);
...
void *handle_client(void *arg){
    ...
    while ((rlen = read(cli->connfd, buff_in, sizeof(buff_in) - 1)) > 0) {
        ...
        if (buff_in[0] == '/') {
            ...
            if (!strcmp(command, "/quit")) {
            ...
            } else if (!strcmp(command, "/cgp")) {
                param = strtok(NULL, " ");
                if (param) {
 

					groups *gp = (groups *)malloc(sizeof(groups));
                    
                    pthread_mutex_lock(&topic_mutex);
                    
                    strcat(gp->name, param);
                    //strcat(topic, " ");
					strcat(gp->owner, cli->name);
                    //strcat(gp->owner, " ");
                    sprintf(buff_out, "<<  group %s created   \r\n", gp->name);
                    //sprintf(buff_out, "<<  group %s owner is   \r\n", gp->owner);
                    gp->connfd = cli->connfd;
                    send_message_all(buff_out);
                    gp->gid = gid++;
        			gp_count++;
					...
					/*memcpy(gp->name, param, 32);
					gp->name[31] = 0;
					memcpy(gp->owner, cli->name, 32);
					gp->owner[31] = 0;
					gp->connfd = cli->connfd;
					gp->gid = gid++;
        			gp_count++;
					*/
                    ...
                    char *gp_name = _strdup(param);
                    sprintf(buff_out, "<< %s created a gp and named it : %s\r\n", cli->name, gp_name);
                    free(gp_name);
                    send_message_all(buff_out);    
                } else {
                    send_message_self("<< name cannot be null\r\n", cli->connfd);
                ...
                if (param) {
					...
					for (int i = 0; i < MAX_GROUPS ; ++i){
                		if (!strcmp(gps[i]->name, param)) {
                			sprintf(buff_out, "joined to %s group ", gps[i]->name);
                			send_message_self(buff_out, cli->connfd);
                        }				    
				    }    	//cli->gp_ID = gps[i]->gid;
				}			
                ...
                /*	for (int i = 0; i < MAX_GROUPS ; ++i){
				    ...
				    sprintf(buff_out, "to  gp %s members", param);
                    while (param != NULL) {
                            strcat(buff_out, " ");
                            strcat(buff_out, param);
                            param = strtok(NULL, " ");
                    ...
                    strcat(buff_out, "\r\n");
                    send_message_to_gp_members (buff_out, cli->gp_ID);
				        
                }}*/else if(!strcmp(command, "/list")) {
                	sprintf(buff_out, "<< clients %d\r\n", cli_count);
                	send_message_self(buff_out, cli->connfd);
                	send_active_clients(cli->connfd);
            	}else if(!strcmp(command, "/lgp")) {
                	sprintf(buff_out, "<< groups %d\r\n", gp_count);
                	//sprintf(buff_out, "<< gps[0] : %s\r\n", gps[0]->name);
                	send_message_self(buff_out, cli->connfd);
                	send_active_groups(cli->connfd);
            	} else if (!strcmp(command, "/help")) {
                	strcat(buff_out, "<< /quit     Quit chatroom\r\n");
                	strcat(buff_out, "<< /cgp      Create a group\r\n");
                	strcat(buff_out, "<< /lgp      shows list of groups\r\n");
                	strcat(buff_out, "<< /lgp      shows list of groups\r\n");
